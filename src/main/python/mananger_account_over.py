...
class Btn_facebook_action(QtWidgets.QPushButton):
    ...
    def update_disable(self,flag:bool):
        ...
        response = requests.put(f"{BASE_URL}/update_account_by_uid", headers=headers,json=data_js)
...
class Mananger_account(Ui_Mananger_Account):
    ...
    def load_data(self):
        ...
        BASE_URL = _TNITBEST321JS.get("BASE_URL")
        response = requests.get(f"{BASE_URL}/get_all_my_facebook_account", headers=headers)
        self.data_js = {}
        ...
        if self.data_js.get("data"):
            ...
            for row,data in enumerate(self.data_js.get("data")):
                col = 0
                action_facebook_button = Btn_facebook_action()
                action_facebook_button.setFixedWidth(150)
                self.gridLayout_list_account.addWidget(action_facebook_button,row,col,Qt.AlignLeft)
                uid = None
                for cookie in data["cookies"]:
                    if cookie.get("name") ==  "c_user":
                        uid = cookie.get("value")
                        break
                action_facebook_button.set_uid(uid = str(cookie.get('value')),father=self.Form,ctx=self.ctx)
                action_facebook_button.set_current_window(self)
                action_facebook_button.set_type(_type = Btn_facebook_action.TYPE_LOGIN)
                action_facebook_button.clicked.connect(action_facebook_button.action)   
                ...
                self.gridLayout_list_account.addWidget(lable,row,col,Qt.AlignLeft)
                ...
                if data.get("is_disabled")==False:
                    action_facebook_button.set_type(_type = Btn_facebook_action.TYPE_UPDATE_DISABLE)
                    action_facebook_button.setText(f"Disable")
                    self.set_color_button(action_facebook_button,0,100,0)
                else:
                    action_facebook_button.set_type(_type = Btn_facebook_action.TYPE_UPDATE_ENABLE)
                ...
                if data.get("is_cooldown")==False:
                    action_facebook_button.set_type(_type = Btn_facebook_action.TYPE_UPDATE_COOLDOWN)
                    action_facebook_button.setText(f"Cooldown")
                    self.set_color_button(action_facebook_button,0,100,0)
                else:
                    action_facebook_button.set_type(_type = Btn_facebook_action.TYPE_UPDATE_UNCOOLDOWN)
                ...
                time_remain.setFixedWidth(180)
                self.gridLayout_list_account.addWidget(time_remain,row,col,Qt.AlignLeft)
                time_remain.setText(f"Time cooldow:{data.get('remain_time_cool_down')}")
    ...
    def login_to_system(self,this_btn):
        print(f"{int(time.time())}:Login with uid: "+self.login_map[this_btn.__hash__])
